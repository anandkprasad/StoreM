...
app.get('/pdf/:id', async (req, res) => {
  ...
  // strip .pdf suffix for lookups
  const base = raw.replace(/\.pdf$/i, '');
  const filename = base + '.pdf';
  const dropboxPath = `/store-pdfs/${filename}`;

  if (!DROPBOX_ACCESS_TOKEN) {
    console.error('[pdf proxy] Missing DROPBOX_ACCESS_TOKEN');
    return res.status(500).send('Server misconfigured');
  }

  try {
    // 1) If base looks like an ObjectId, try to find Item by ID (strip .pdf)
    let item = null;
    if (/^[0-9a-fA-F]{24}$/.test(base)) {
      try {
        item = await Item.findById(base).lean();
        if (item) console.log('[pdf proxy] Found item by _id:', base);
      } catch (err) {
        console.warn('[pdf proxy] findById error:', err);
      }
    }

    // 2) If no item yet, try to find item by filename match in pdf_url
    if (!item) {
      try {
        // escape regex special chars in filename
        const esc = filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        item = await Item.findOne({ pdf_url: { $regex: esc } }).lean();
        if (item) console.log('[pdf proxy] Found item by pdf_url match for', filename);
      } catch (err) {
        console.warn('[pdf proxy] DB search by pdf_url failed:', err);
      }
    }

    // 3) If we have an item and it has a pdf_url, proxy that URL (shared link or public)
    if (item && item.pdf_url) {
      console.log('[pdf proxy] Proxying item.pdf_url:', item.pdf_url);
      try {
        // convert some common Dropbox shared link forms to raw
        let proxyUrl = item.pdf_url.replace('?dl=0', '?raw=1').replace('?dl=1', '?raw=1');
        // if it looks like a dropbox sharing URL, prefer using it directly
        const sharedResp = await fetch(proxyUrl);
        if (sharedResp.ok) {
          res.setHeader('Content-Type', sharedResp.headers.get('content-type') || 'application/pdf');
          if (sharedResp.headers.get('content-length')) res.setHeader('Content-Length', sharedResp.headers.get('content-length'));
          return sharedResp.body.pipe(res);
        } else {
          console.warn('[pdf proxy] shared link fetch failed:', sharedResp.status);
        }
      } catch (err) {
        console.warn('[pdf proxy] Error proxying item.pdf_url:', err);
      }
    }

    // 4) Try Dropbox SDK filesDownload for the canonical path
    try {
      console.log('[pdf proxy] Trying dbx.filesDownload for', dropboxPath);
      const sdkResp = await dbx.filesDownload({ path: dropboxPath });
      // sdkResp.result.fileBinary or sdkResp.fileBinary depending on SDK version
      const maybeBinary = sdkResp && (sdkResp.result ? sdkResp.result.fileBinary : sdkResp.fileBinary);
      if (maybeBinary) {
        const buf = Buffer.isBuffer(maybeBinary) ? maybeBinary : Buffer.from(maybeBinary, 'binary');
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Length', buf.length);
        return res.send(buf);
      }
      // else fallthrough to content API
    ...
    // 5) Try the content API (stream)
    try {
      console.log('[pdf proxy] Trying content API for', dropboxPath);
      ...
      // If content API failed, read body for debug
      const bodyText = await resp.text().catch(()=>'<no-body>');
      console.error('[pdf proxy] Dropbox content API error', resp.status, bodyText);

      // If not found, we will attempt to list folder for diagnostics below
    } catch (err) {
      console.error('[pdf proxy] content API error:', err);
    }

    // 6) Diagnostic: list folder /store-pdfs safely
    try {
      console.log('[pdf proxy] Listing /store-pdfs for debugging...');
      const listResp = await dbx.filesListFolder({ path: '/store-pdfs' });
      if (listResp && listResp.result && Array.isArray(listResp.result.entries)) {
        console.log('[pdf proxy] /store-pdfs contains:', listResp.result.entries.slice(0,200).map(e=>e.name));
      } else if (listResp && Array.isArray(listResp.entries)) {
        console.log('[pdf proxy] /store-pdfs contains:', listResp.entries.slice(0,200).map(e=>e.name));
      } else {
        console.log('[pdf proxy] /store-pdfs entries response shape unexpected, dump:', listResp);
      }
    } catch (listErr) {
      console.warn('[pdf proxy] Could not list /store-pdfs (check token & permissions):', listErr && (listErr.error_summary || listErr));
    }

    // Final: inform client the file was not found, point to logs
    return res.status(404).send('File not found in Dropbox. Check server logs for /store-pdfs listing and DB entry.');
  } catch (err) {
  ...
  try {
    ...
    app.get('/notes/:orderId/view', isLoggedIn, async function(req, res) {
      ...
      res.render('pdf-viewer', {
        // request the Dropbox file path built from the product._id (this file exists)
        pdfUrl: `/pdf/${product._id}.pdf`,  
        user: req.user,
        orderId: order._id
      });
