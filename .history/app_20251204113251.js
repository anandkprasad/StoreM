...
app.get('/notes/:orderId/page/:pageNo/image', async (req, res) => {
  try {
    ...
    // build signed Cloudinary fetch URL
    ...
    const signedUrl = cloudinary.url(product.pdf_url, {
      type: 'fetch',
      resource_type: 'image',
      ...
      transformation: [
        { overlay: { text: watermarkText, font_family: 'Arial', font_size: 18, font_weight: 'bold', opacity: 40, color: '#000000' }, gravity: 'south_east', x: 20, y: 20 }
      ]
    });

    console.log('[DBG] returning signed url (truncated):', typeof signedUrl === 'string' ? signedUrl.slice(0,120) + '...' : signedUrl);
...
app.get('/notes/:orderId/page/:pageNo/image', async (req, res) => {
  try {
    ...
    // normalize Dropbox share link to a direct-download URL Cloudinary can fetch
    function normalizeDropboxUrl(u) {
      try {
        if (!u || !u.includes('dropbox.com')) return u;
        const [base, qs] = u.split('?');
        const params = new URLSearchParams(qs || '');
        // prefer raw=1 (works well for direct fetch)
        params.set('raw', '1');
        // remove dl param to avoid confusion
        params.delete('dl');
    ...
    const rawPdfUrl = normalizeDropboxUrl(product.pdf_url);

    const signedUrl = cloudinary.url(rawPdfUrl, {
      type: 'fetch',                  // convert from Dropbox URL on-the-fly
      resource_type: 'image',
      page: page,
      format: 'jpg',
      quality: 'auto',
