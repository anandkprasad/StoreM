...
app.get('/pdf/:id', async (req, res) => {
  ...
  let id = rawId.endsWith('.pdf') ? rawId : rawId + '.pdf';
  const dropboxPath = `/store-pdfs/${id}`;

  if (!DROPBOX_ACCESS_TOKEN) return res.status(500).send('Server misconfigured: missing Dropbox token');

  try {
    // 1) Try SDK download (best effort)
    try {
      ...
      // Some SDK versions return a readable stream or different shape — fall through to content API
      console.log('[pdf proxy] filesDownload returned no binary, falling back to content API');
    } catch (sdkErr) {
      // If it's a path/not_found, we'll attempt fallbacks below. Log full sdkErr for debugging.
      console.warn('[pdf proxy] filesDownload error:', sdkErr && (sdkErr.status || sdkErr.error || sdkErr));
      // continue to content API
    }

    // 2) Try content API (stream)
    ...
    // If we reach here, content API failed. Inspect body for Dropbox error.
    let bodyText = '<no-body>';
    try { bodyText = await resp.text(); } catch(e) {}
    console.error('[pdf proxy] Dropbox content API error', resp.status, bodyText);

    // 3) If path not found (409), try to fallback by looking up the Item in DB and using stored shared link
    if (resp.status === 409) {
      console.log('[pdf proxy] path not found. Attempting DB fallback for id:', req.params.id);
      try {
        const item = await Item.findOne({ _id: req.params.id }).lean();
        if (item && item.pdf_url) {
          console.log('[pdf proxy] Found item.pdf_url:', item.pdf_url);
          // If it's a Dropbox shared link, prefer Dropbox sharing endpoint
          if (item.pdf_url.includes('dropbox.com')) {
            // Convert shared link to raw (if classic /s/ link) or use sharing/get_shared_link_file
            // If the link contains dl=0, replace with raw=1
            let rawLink = item.pdf_url.replace('?dl=0', '?raw=1').replace('?dl=1', '?raw=1');
            console.log('[pdf proxy] Proxying shared-link raw URL:', rawLink);
            const sharedResp = await fetch(rawLink);
            if (sharedResp.ok) {
            ...
            // Non-dropbox public URL — proxy it
            const publicResp = await fetch(item.pdf_url);
            if (publicResp.ok) {
              res.setHeader('Content-Type', publicResp.headers.get('content-type') || 'application/pdf');
              if (publicResp.headers.get('content-length')) res.setHeader('Content-Length', publicResp.headers.get('content-length'));
              return publicResp.body.pipe(res);
            }
          }
        } else {
          console.log('[pdf proxy] No item.pdf_url found for', req.params.id);
        }
      } catch (dbErr) {
        console.error('[pdf proxy] DB fallback error:', dbErr);
      }
    }

    // 4) Final helpful response
    // As a last resort, list the /store-pdfs folder contents for debugging
    try {
      const list = await dbx.filesListFolder({ path: '/store-pdfs' });
      console.log('[pdf proxy] /store-pdfs entries:', list.entries.map(e=>e.name).slice(0,50));
    } catch(listErr) {
      console.warn('[pdf proxy] Could not list /store-pdfs:', listErr && listErr.error_summary || listErr);
    }

    res.status(502).send('Failed to retrieve file from Dropbox. See server logs for details.');
  } catch (err) {
    console.error('[pdf proxy] Unexpected error:', err);
    res.status(500).send('PDF proxy failed');
  }
});
