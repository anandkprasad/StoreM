...
app.get('/notes/:orderId/page/:pageNo/image', async (req, res) => {
  ...
  const processFiles = async () => {
    // Multiple images support: upload each image to Cloudinary and store URLs
    if (req.files && req.files.images && req.files.images.length) {
      ...
      try {
        ...
        // Use first image as primary thumbnail and keep all images in page_images
        imageUrl = urls[0];
        pageImages = urls;
      } catch (err) {
        console.error('[admin/add] image upload error', err);
        throw err;
      }
    }
