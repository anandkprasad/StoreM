...
// Note: This requires pdf-poppler or similar library for server-side conversion
// For now, we'll upload PDF and convert pages on-demand using Cloudinary
async function processPDFToImages(pdfBuffer, itemId) {
  return new Promise((resolve, reject) => {
    // Upload PDF to Cloudinary as private resource
    cloudinary.uploader.upload_stream(
      {
        resource_type: 'raw',
        folder: `store-pdfs/${itemId}`,
        type: 'upload',
        access_mode: 'authenticated', // Private, not publicly accessible
        format: 'pdf',
        use_filename: true,
      ...
      async (error, pdfResult) => {
        ...
        // Store PDF URL and public_id
        // Pages will be converted on-demand when viewing
        // For now, we'll store the PDF and convert pages when needed
        resolve({
          pdfUrl: pdfResult.secure_url,
          pdfPublicId: pdfResult.public_id,
          pageImages: [] // Will be populated on first view or via background job
        });
      }
    ).end(pdfBuffer);
  });
...
app.post('/admin/add', isLoggedIn, isAdmin, upload.fields([
  ...
  // Process PDF if digital product
  const processPDF = async (itemId) => {
    if (isDigital && req.files && req.files.pdf && req.files.pdf[0]) {
      try {
        const pdfFile = req.files.pdf[0];
        const pdfResult = await processPDFToImages(pdfFile.buffer, itemId);
