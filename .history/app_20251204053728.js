...
app.get('/', async function (req, res) {
  try {
    // Fetch featured items (only 2)
    const featuredItems = await Item.find({ featured: true }).limit(2);

    // Fetch all non-featured items
    const otherItems = await Item.find({ featured: false });
