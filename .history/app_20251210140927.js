...
app.get('/notes/:orderId', isLoggedIn, async function(req, res) {
  try {
      console.log(`[notes/:orderId] Route entered for user=${req.user ? req.user.username : 'Guest'}, orderId=${req.params.orderId}`);
    // Find the order
    ...
    const firstPagePath = path.join(productDir, 'page-001.jpg');
        console.log(`[notes/:orderId] Checking/triggering image conversion: pdf_url=${product.pdf_url}, page1 exists=${fs.existsSync(firstPagePath)}, productId=${product._id.toString()}`);
...
app.get('/search', async (req, res) => {
  ...
  let recommended = [];
  if (query) {
          console.log(`[notes/:orderId/view] Checking/triggering image conversion: pdf_url=${product.pdf_url}, page1 exists=${fs.existsSync(firstPagePath)}, productId=${product._id.toString()}`);
