...
app.get('/notes/:orderId', isLoggedIn, async function(req, res) {
  try {
    ...
    if (product.pdf_url && !fs.existsSync(firstPagePath)) {
      console.log('[notes/:orderId] Triggering convertDropboxPdfToImagesLocal for', product.pdf_url, 'productId=', product._id);
      await convertDropboxPdfToImagesLocal(product.pdf_url, 10, product._id); 
    }
    ...
    app.get('/notes/:orderId/view', isLoggedIn, async function(req, res) {
      ...
      if (product.pdf_url && !fs.existsSync(firstPagePath)) {
        console.log('[notes/:orderId/view] Triggering convertDropboxPdfToImagesLocal for', product.pdf_url, 'productId=', product._id);
        await convertDropboxPdfToImagesLocal(product.pdf_url, 10, product._id); 
      }
...
app.get('/notes/:orderId/page/:pageNo/image', async (req, res) => {
  try {
    ...
    // Build local image path under ./out-pages/<productId> (generated by convertDropboxPdfToImagesLocal)
    ...
    if (!fs.existsSync(localPath)) {
      console.warn('[DBG] requested local page image missing:', localPath);
...
app.get('/notes/:orderId/page/:pageNo', isLoggedIn, async function(req, res) {
  try {
    ...
    if (product.pdf_url && !fs.existsSync(pagePath)) {
      // Convert up to the requested page so user experience is smooth
      console.log(`[notes/:orderId/page/:pageNo] For order ${orderId}, page ${pageNumber}: image not found for product ${product._id}, running convertDropboxPdfToImagesLocal for first ${pageNumber} pages.`);
      await convertDropboxPdfToImagesLocal(product.pdf_url, Math.max(pageNumber, 10), product._id);
    }
...
app.post('/admin/add', isLoggedIn, isAdmin, upload.fields([
  ...
  var isDigital = true; // Always true for digital products
  var imageUrl = '';
  var pdfUrl = '';
  var pageImages = [];

  // Truncate description to 255 characters for Razorpay compatibility
  ...
  async function saveItem() {
    try {
      var newItem = new Item({
        ...
        image_url: imageUrl,
        is_digital: isDigital, // Always true
        pdf_url: pdfUrl,
  ...
  // (handle image and PDF uploads, then call saveItem())
